  ...
  <xsl:template match="/">
    <xsl:element name="data">
      ...
      <xsl:for-each select="//record/@*">
        <xsl:if test="not(starts-with(name(),'jsx') or starts-with(name(),'hasChanges') or starts-with(name(),'oldId'))">
	        <xsl:element name="record">
		        <xsl:attribute name="jsxid"><xsl:value-of select="generate-id()"/></xsl:attribute>
		        <xsl:attribute name="jsxtext">{typeConfig_<xsl:value-of select="name()"/>}</xsl:attribute>
